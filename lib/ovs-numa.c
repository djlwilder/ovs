 ...
 * There is no guarantee that Node and cpu ids are numbed consecutively (this is a change
 * from earlier version of the code).  So, for example, if two nodes exist with ids
 * 0 and 8, 'ovs_numa_get_n_nodes()' will return 2, no assumption of node numbering should be made.
 *
 * NOTE, this module should only be used by the main thread.
 *
 * NOTE, if cpu hotplug is used 'all_numa_nodes' and 'all_cpu_cores' must be invalidated
 * when ever the system topology changes. Support for detecting topology changes has not been included.
 * For now, add a TODO entry for addressing it in the future.
 *
